...
4. The link appears in the live `VtoStylist` success state with a result image; it is absent from idle, pending, error, and reupload states. *Amended 2026-08-19 by Story 2.8: the original clause also excluded VTO-history and the Feed, which made a successful (2-unit) YouCam generation the only route to any product page. Buy Now now also appears in the Past Try-Ons full-image viewer and on Feed cards; it remains absent from the non-success VTO states listed above and from the AI Stylist trend picker.*
