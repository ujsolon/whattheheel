...
- [ ] [Review][Patch] **Palette hardcoded as hex literals; border color is wrong** — `#d4ff3f`, `#ff3ec9`, `#0a0a0a`, `#151515`, `#242424` are inlined across five components plus `globals.css`, while the existing `@theme inline` block goes unextended. Separately every structural border uses `border-black` (`#000000`) while its paired shadow uses `#0a0a0a`, so `border-ink` is two different colors inside one component. [app/components/*.tsx, app/loading.tsx, app/globals.css]
- [ ] [Review][Patch] **Card label has no wrap guard** — `min-h-16 px-3 py-2` with no `break-words`/`hyphens`/`line-clamp`; a long unbroken label overflows the 3px card border. The task said "without allowing long text to break the grid." [app/components/TrendCard.tsx:21-23]
- [ ] [Review][Patch] **LCP images are lazy-loaded** — all three 1.2–1.5 MB PNGs are above the fold at `lg`+ but no `priority`/`fetchPriority` is set, deferring the LCP element. [app/components/TrendCard.tsx:11-16]
- [ ] [Review][Patch] **Two component tests weaker than the stated requirement** — `TrendFeed.test.tsx` and `Marquee.test.tsx` assert only `getByText`; the empty-state test never asserts the `role="status"` region it relies on, and nothing asserts `aria-label="Trending shoes"`. Dev Notes require "meaningful role/name assertions." [app/components/__tests__/TrendFeed.test.tsx, app/components/__tests__/Marquee.test.tsx]
