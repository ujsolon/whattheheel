...
Code review 2026-08-17 (Blind Hunter + Edge Case Hunter + Acceptance Auditor). Severity reflects real user consequence after verifying reachability against the built output.

- [x] [Review][Defer] **`app/loading.tsx` is unreachable dead code** [app/loading.tsx] — deferred: kept deliberately as scaffolding for Story 1.3+, when real interaction makes the Suspense boundary reachable. Confirmed unreachable today — `Home()`/`getTrends()` are synchronous, `/` is `"compute": "static"` with content baked into `.next/server/app/index.html`, and no `next/link` exists to trigger client navigation. Task 5 stays checked on that basis; re-verify it actually renders once Story 1.3 lands.
- [x] [Review][Defer] **DESIGN.md typography scale not implemented** [app/page.tsx, app/components/*.tsx, app/globals.css] — deferred: revisit at UX finalize, where the token values themselves are still open. `page.tsx` h1 is `text-4xl sm:text-5xl` (36/48px) vs the spec's 26px `display` token; marquee 12/14px vs 11px; card headings 14/16px vs 13px; no `'Arial Black'` display stack in `globals.css`. Resolve by either conforming the code or writing the shipped values back into DESIGN.md — do not leave the two silently diverged.
- [x] [Review][Patch] **Mobile DOM order violates AC6** — `<AppNavigation />` is the first child of `page.tsx` but renders `fixed bottom-0` below `lg`, so AT/keyboard users traverse the nav before the marquee, `<h1>`, and feed. AC6 requires reading order to follow visual order. [app/page.tsx:11, app/components/AppNavigation.tsx:7]
- [x] [Review][Patch] **`env(safe-area-inset-bottom)` always resolves to 0** — `app/layout.tsx` exports `metadata` only, no `viewport`. Without `export const viewport = { viewportFit: "cover" }` the browser never enters edge-to-edge mode, so both safe-area paddings are inert and the fixed nav can sit under the home indicator. [app/layout.tsx, app/components/AppNavigation.tsx:7, app/page.tsx:13]
- [x] [Review][Patch] **`aria-disabled` on a bare `<span>` is inert ARIA** — `role=generic` does not support `aria-disabled`, so AT ignores it, and the inactive tabs are visually identical to a normal inactive tab. Neither sighted nor AT users learn AI Stylist/Profile are unavailable, contradicting the guardrail "represent as *clearly unavailable*". The test asserts the broken attribute, cementing it. [app/components/AppNavigation.tsx:19-21]
- [x] [Review][Patch] **Marquee loop is not seamless** — all three reviewers independently derived this. The keyframe's `calc(-50% - 1.5rem)` wrap point is only correct for two byte-identical children, but span two is prefixed `★ `, making it wider by δ. The ticker snaps ~δ/2 out of register every 18s. [app/globals.css @keyframes marquee-scroll, app/components/Marquee.tsx:7-9]
- [x] [Review][Patch] **Marquee is a phantom tab stop** — `tabIndex={0}` on a `<div>` with no role or accessible name makes decorative ticker copy the page's only tab stop; under `prefers-reduced-motion` the pause it enables is already a no-op. [app/components/Marquee.tsx:5]
- [x] [Review][Patch] **Validated `shoeImageUrl` can still throw and take down the whole route** — `isTrend` only checks non-empty string, but `next/image` hard-rejects remote hosts (no `images` config), protocol-relative URLs, and paths without a leading slash. Subtler: `isNonEmptyString` calls `.trim()` for the check but `trends.push(entry)` stores the raw value, so `" /trends/x.png"` passes validation then throws. No `app/error.tsx` exists, so one bad seed row fails the entire build. This defeats the purpose of the skip-invalid-entries path. [lib/data/trends.ts:14-30, app/components/TrendCard.tsx:11-16]
- [x] [Review][Patch] **Skip-entry logging is undiagnosable** — `console.error("Skipping invalid or duplicate trend entry")` carries no id, no index, and no failing field, and uses identical text for two unrelated failure modes. AC5 requires logging the underlying error. Motivating case: a curator who omits `buyUrl` entirely (the natural JSON spelling of "no link") gets `undefined`, fails both guard arms, and silently loses the trend with an untraceable log line. [lib/data/trends.ts:44-46]
- [x] [Review][Patch] **No test exercises the real seed** — `page.test.tsx` mocks `@/lib/data/trends` and `trends.test.tsx` mocks `node:fs`, so nothing parses the actual `public/trends.json` or asserts the three `shoeImageUrl` values resolve to files on disk. The single highest-probability defect in this story — a filename typo — ships green. [app/__tests__/page.test.tsx:4-13, lib/data/__tests__/trends.test.tsx:5]
- [x] [Review][Patch] **Palette hardcoded as hex literals; border color is wrong** — `#d4ff3f`, `#ff3ec9`, `#0a0a0a`, `#151515`, `#242424` are inlined across five components plus `globals.css`, while the existing `@theme inline` block goes unextended. Separately every structural border uses `border-black` (`#000000`) while its paired shadow uses `#0a0a0a`, so `border-ink` is two different colors inside one component. [app/components/*.tsx, app/loading.tsx, app/globals.css]
- [x] [Review][Patch] **Card label has no wrap guard** — `min-h-16 px-3 py-2` with no `break-words`/`hyphens`/`line-clamp`; a long unbroken label overflows the 3px card border. The task said "without allowing long text to break the grid." [app/components/TrendCard.tsx:21-23]
- [x] [Review][Patch] **LCP images are lazy-loaded** — all three 1.2–1.5 MB PNGs are above the fold at `lg`+ but no `priority`/`fetchPriority` is set, deferring the LCP element. [app/components/TrendCard.tsx:11-16]
- [x] [Review][Patch] **Two component tests weaker than the stated requirement** — `TrendFeed.test.tsx` and `Marquee.test.tsx` assert only `getByText`; the empty-state test never asserts the `role="status"` region it relies on, and nothing asserts `aria-label="Trending shoes"`. Dev Notes require "meaningful role/name assertions." [app/components/__tests__/TrendFeed.test.tsx, app/components/__tests__/Marquee.test.tsx]
- [x] [Review][Defer] **Static prerender freezes the seed while `/trends.json` serves a live copy** [lib/data/trends.ts:11,34] — deferred. Editing the seed in production updates the public URL instantly and the rendered feed not at all (requires rebuild). Currently correct-by-accident for a hand-curated hackathon seed; becomes a live bug the moment the route turns dynamic, where `public/` is CDN-served and may not exist at `process.cwd()` in a serverless function.
- [x] [Review][Defer] **Loading skeleton drifts from card geometry and the design token** [app/loading.tsx] — deferred: `loading.tsx` is being kept as unreachable scaffolding (see above), so polishing it now is premature. Must be fixed before Story 1.3 makes the boundary reachable. 8 skeletons vs 3 seed entries, `aspect-[4/5]` vs the real `aspect-square` + `min-h-16`, dark stripes vs DESIGN.md's light `#E7E7E7`/`#D2D2D2` shimmer, and no `AppNavigation`/`Marquee` so chrome would pop in.
- [x] [Review][Defer] **`lg:sticky` leaves `bottom-0` unreset** [app/components/AppNavigation.tsx:7] — deferred, cosmetic robustness. A sticky box with both `top` and `bottom` set is fragile; `lg:bottom-auto` should be explicit. Verified rendering acceptably at 1440×1000 today.
