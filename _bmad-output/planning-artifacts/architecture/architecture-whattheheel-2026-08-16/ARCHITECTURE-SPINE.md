...
| State & cross-cutting (mutation, errors, logging, config, auth) | Mongo writes happen only inside `lib/data/*`, called only from `lib/services/*` — Route Handlers never call the driver directly. **Exception:** the `users` collection's core auth fields (`id`, `email`, `emailVerified`, `image`) are owned and written exclusively by NextAuth's MongoDB adapter, mounted in `lib/services/auth.ts` — no other code writes to `users`. App-specific profile data (e.g. `selfieUrl`) lives in a separate `user_profiles` collection, keyed by `userId`, written only via `lib/data/userProfiles.ts`. Auth session read via NextAuth's server-side session helper, never trusted from a client-supplied value. Server errors logged via `console.error` (per project-context.md); user-facing copy always distinct from the logged message. All secrets via environment variables only, never hardcoded, never `NEXT_PUBLIC_`-prefixed (see AD-1). |
...
| next-auth | 4.24.15 (peer dep confirms Next.js 16 support; v5/`next-auth@5.0.0-beta.32` deliberately rejected — still beta) |
| @next-auth/mongodb-adapter | 1.1.3 (last published 2023 — stale but is the correct v4-targeted adapter; `@auth/mongodb-adapter` targets v5+ and is known-incompatible with v4. Install with `--legacy-peer-deps`.) |
...
lib/
  ...
  data/
    userProfiles.ts                # user_profiles collection repository (app-owned fields only; `users` itself is NextAuth-adapter-owned)
...
| FR-03 Registration/Profile | `app/api/auth/*`, `app/api/upload/route.ts`, `lib/data/userProfiles.ts` | AD-4, AD-5 |
...
- **NextAuth session strategy (JWT vs. database sessions)** — v4 supports both; pick at implementation time based on whether Edge middleware ends up needed anywhere (it currently isn't, per AD-1).
...
- **Polling UX during VTO inference** (PRD Performance NFR: "the UI must manage user expectations during inference") — a UI/copy concern for the stylist result component, not an architectural invariant; left to implementation.
