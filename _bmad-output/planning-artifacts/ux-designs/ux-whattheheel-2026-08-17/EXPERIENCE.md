...
| Trend feed card (`{components.trend-card}`) | Feed grid | Tap opens Overlay Preview with that trend pre-loaded. Carries a **secondary** Buy Now action when the trend has a `buyUrl` (amended 2026-08-19, Story 2.8 — the original "no secondary action" rule predated retail links and forced every route to a product page through a paid VTO generation). The two actions are siblings inside the card, never nested, since an `<a>` inside an `<a>` is invalid HTML. Long-press/hover: none. Badge shows at most one of HOT / NEW / a 🔥 fire-count badge (e.g. "🔥 12") — never stacked. |
...
| Inline error component (`{components.inline-error}`) | AI Stylist → VTO Failure | Renders inline on the **same screen** as the trigger/result, never a modal (FR-06, AD-6). Shows the exact locked copy for the returned code (see Voice and Tone). The recovery action depends on **whose fault the failure is** (clarified 2026-08-18, Story 2.4 review — the earlier "always offers Try another photo" wording forced a re-upload even when the photo was blameless): **photo-fault** codes (`error_no_face`, `error_download_image`, `error_nsfw_content_detected`, `exceed_max_filesize`) offer "Try another photo", re-opening the Selfie Upload control in place. **System-fault** failures (`error_inference`, `invalid_parameter`, unmapped codes, the durable-copy failure from Story 2.6) offer "Try again" as the primary action — returning to the trigger without demanding a new file — with "Try another photo" kept as a secondary action. **Connection** failures (a failed poll request, or the 90s timeout — see State Patterns) offer "Retry", which re-polls the same task rather than creating a new billable one. In every case the user stays on the AI Stylist surface, and the re-upload control itself always offers a way back out ("Keep my current photo") so a failing upload is never a dead end. |
| Buy Now link (`{components.buy-now-button}`) | VTO Result; Feed card; Past Try-Ons full-image viewer (all three since Story 2.8 — reaching a retailer must never require spending a YouCam generation) | Renders only when the trend's `buyUrl` is present; **hidden entirely** (not disabled/grayed) when absent — never a dead or broken-looking link (Story 2.5). Opens in a new tab (`target="_blank" rel="noopener"`). |
