...
| Profile | Bottom/top nav | Account summary, selfie on file, sign out | Requires login |
...
| Upload control (`{components.upload-dropzone}`) | Selfie Upload; foot-photo picker inside Overlay Preview | Native `<input type="file" accept="image/*">` — no custom camera UI (memlog decision), so mobile browsers surface the OS camera option for free. Selfie path: file is validated **server-side** before any Cloudinary upload (AD-5) — client-side checks, if present, are a UX hint only and never authoritative. On a validation failure the control shows an inline message in the plain register (e.g. wrong format/too large) directly below the dropzone and does not proceed to upload; the user picks a new file from the same control without navigating away. |
...
- **File upload** — `<input type="file" accept="image/*">`, click-to-browse; on mobile this surfaces the OS's own camera-or-library chooser. No bespoke camera capture UI anywhere in the product.
